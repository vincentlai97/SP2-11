...
)
{
	...
	for (unsigned count = 0; count < vertexIndices.size(); count++)
	{
		// Get the indices of its attributes
		unsigned int vertexIndex = vertexIndices[count];
		unsigned int uvIndex = uvIndices[count];
		unsigned int normalIndex = normalIndices[count];
